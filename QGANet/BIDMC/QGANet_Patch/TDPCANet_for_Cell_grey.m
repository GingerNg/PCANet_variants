...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.') && ~strcmp(subname, '..'),
        ...
        frames = dir(fullfile(rt_Img_dir, subname, '*.bmp'));% .mat 文件的路径;
        ...
        for jj = 1:c_num,   % c_num*length(subfolders)= 图片总数;
            imgpath = fullfile(rt_Img_dir, subname, frames(jj).name);% frames（jj）.name的路径
             fprintf('Processing %s: \n', ...
                     frames(jj).name);
            I = imread(imgpath);
            I_cell ={rgb2gray(im2double(I))};
            feaSet.feaArr = [feaTDPCANet_FeaExt(I_cell,V,PCANet)]; % extract a test feature using trained PCANet model
