function [f, BlkIdx] = HistreplaceSc(PCANet,ImgIdx,OutImg)
...
for Idx = 1:NumImg %图
    ...
    for i = 1:NumImginO   % 8
        ...
        for j = 1:PCANet.NumFilters(end) % 8
            T = T + map_weights(j)*Heaviside(OutImg{Idx_span(PCANet.NumFilters(end)*(i-1)+j)}); % T ：十进制的数
            % weighted combination; hashing codes to decimal number conversion
            
            OutImg{Idx_span(PCANet.NumFilters(end)*(i-1)+j)} = []; % 清空已经二值化的数据。
        end
