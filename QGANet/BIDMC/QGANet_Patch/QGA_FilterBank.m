...
dat=[];
for i = RandIdx 
    im = im2patch(InImg{i},11,PatchSize,1); % collect all the patches of the ith image in a matrix
    % sampling one image
    num_per_img = round(NumRSamples/ImgZ);  %??
    
     load('E:\PCANet\Data\M0\fold5\a');
    patchIdx = [a{i}];
...
%      Rx = Rx + PLUSq(im,im);
 Rx = Rx + qmatrix_mul(im,permute(im,[2 1 3]));
end
