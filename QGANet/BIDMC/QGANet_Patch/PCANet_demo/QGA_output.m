...
for i = 1:ImgZ
    ...
    img((mag+1):end-mag,(mag+1):end-mag,:) = InImg{i}; % 补零
    for k = 1:size(img,3)
    % collect all the patches of the ith image in a matrix
    im(:,:,k) = im2col_general(img(:,:,k),[PatchSize PatchSize]); 
    end
    ...
    for j = 1:NumFilters  % 8
        ...
        rr = qmatrix_mul(VV,im(:,780001:end,:));%四元数矩阵相乘
        RR = [RR,rr];
