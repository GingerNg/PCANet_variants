...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.') && ~strcmp(subname, '..'),
        ...
        for jj = 1:c_num,   % c_num*length(subfolders)= 图片总数;
            ...
            I = Image2Qua(im2double(imread(imgpath)));
            counter = counter+1;
...
fprintf('\n ====== PCANet Training ======= \n')
[ftrain V BlkIdx] = feaQGANet_train(TrnData_ImgCell,PCANet,0); 
save('E:\QGANet\Data\V','V');
else
...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.') && ~strcmp(subname, '..'),
        ...
        for jj = 1:c_num,   % c_num*length(subfolders)= 图片总数;
            ...
            I_cell ={Image2Qua(I)};  % image to quaternion
            feaSet.feaArr = [feaQGANet_FeaExt(I_cell,V,PCANet)]; % extract a test feature using trained PCANet model
