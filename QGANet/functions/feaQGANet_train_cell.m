function [f V BlkIdx] = feaQGANet_train_cell(InImg,PCANet,IdtExt)  % IdtExt = 1;
% =======INPUT=============
...
NumImg = length(InImg);% 图的个数


V = cell(PCANet.NumStages,1); % 滤波器
OutImg = InImg; 
ImgIdx = (1:NumImg)';
clear InImg; 
...
for stage = 1:PCANet.NumStages
    ...
    % patch_PCA_FilterBank 对patch采样；
    V{stage} = QGA_FilterBank_cell(OutImg, PCANet.PatchSize, PCANet.NumFilters(stage)); % compute PCA filter banks
...
if IdtExt == 1 % enable feature extraction
    ...
    for idx = 1:NumImg % 58
        ...
        % OutImg_i:第idx幅图的64幅图  ImgIdx_i :第idx幅图的8*8个标签
        ...
        [f{idx} BlkIdx] = HistreplaceSc(PCANet,ImgIdx_i,OutImg_i); % compute the feature of image "idx" ; 
