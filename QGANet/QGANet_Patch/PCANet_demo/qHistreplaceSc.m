function [f, BlkIdx] = qHistreplaceSc(PCANet,ImgIdx,OutImg)
...
for Idx = 1:NumImg %图
    ...
    for i = 1:NumImginO   % 8
        ...
        for j = 1:PCANet.NumFilters(end) % 8
            T = T + map_weights(j)*Heaviside(OutImg{Idx_span(PCANet.NumFilters(end)*(i-1)+j)}(:,:,q)); % T ：十进制的数
